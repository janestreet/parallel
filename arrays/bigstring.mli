...
val%template sub_shared : ('a : any). 'a t @ m -> pos:int -> len:int -> 'a t @ m
[@@mode m = (uncontended, shared, contended)]

(** [length t] is the length of [t] in scalars. *)
val length : ('a : any). 'a t @ contended -> int

(** [copy t] duplicates [t]. *)
val%template copy : ('a : any). 'a t @ m -> 'a t @ m
[@@mode m = (uncontended, shared)]
...
val of_string : ('a : any). ?pos:int -> ?len:int -> string -> char t

(** [get t i] loads the scalar at index [i].

    Raises [Invalid_argument] if [i] is out of bounds. *)
val get : ('a : value_or_null). 'a t @ shared -> int -> 'a @ portable

(** [set t i a] stores the scalar [a] to index [i].

    Raises [Invalid_argument] if [i] is out of bounds. *)
val set : ('a : value_or_null). 'a t -> int -> 'a @ portable -> unit

(** [unsafe_get t i] loads the scalar at index [i]. Does not check bounds. *)
val unsafe_get : ('a : value_or_null). 'a t @ shared -> int -> 'a @ portable

(** [unsafe_set t i a] stores the scalar [a] to index [i]. Does not check bounds. *)
val unsafe_set : ('a : value_or_null). 'a t -> int -> 'a @ portable -> unit
...
module Expert : sig
  ...
  val unsafe_racy_get_contended
    : ('a : value_or_null).
    'a t @ contended -> int -> 'a @ contended portable
  ...
  val unsafe_racy_set_contended
    : ('a : value_or_null).
    'a t @ contended -> int -> 'a @ portable -> unit
end
